...
auto model_loader::process_vertices(aiMesh* mesh) const
    ...
    return std::make_shared<vertex_buffer>(format, vertices_deindexed.data(),
                                           vertices_deindexed.size());
}

auto model_loader::process_mesh(aiMesh* mesh, const aiScene* scene,
                                const fs::path& base_path) -> ::mesh {
    // vertices
    ...
    process_textures(material.get(), scene_material, aiTextureType_DIFFUSE,
                     base_path);
    process_textures(material.get(), scene_material, aiTextureType_SPECULAR,
                     base_path);
    /* process_textures(material.get(), scene_material, aiTextureType_HEIGHT,
     * base_path); */
    /* process_textures(material.get(), scene_material, aiTextureType_AMBIENT,
     * base_path); */
...
auto model_loader::deindex_vertices(
    const std::vector<vertex>& vertices,
    const std::vector<unsigned int>& indices) const -> std::vector<vertex> {
